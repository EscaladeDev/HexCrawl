/* ---------------- Defaults ---------------- */
const DEFAULTS = [
  {id:"unknown",label:"Unknown",icon:"❓",color:"#94a3b8",desc:"Nothing distinct; wavering horizon, indistinct shapes.",weight:1},
  {id:"trail",label:"Trail",icon:"👣",color:"#8b5cf6",desc:"Trail sign at the border (ruts, prints, game path).",weight:1},
  {id:"smoke",label:"Smoke",icon:"💨",color:"#9ca3af",desc:"Thin smoke/steam column: fire, forge, geyser, or vent.",weight:1},
  {id:"lights",label:"Lights",icon:"🔥",color:"#f59e0b",desc:"Lights after dusk (camp cluster, patrol lanterns, or eerie glows).",weight:1},
  {id:"water",label:"Water",icon:"💧",color:"#3b82f6",desc:"Water glint or sound: river bend, spray of falls, or lake shine.",weight:1},
  {id:"ruin",label:"Ruin",icon:"🏚️",color:"#7c3aed",desc:"Ruin outline: arch, menhir ring, toppled tower, old wall line.",weight:1},
  {id:"marker",label:"Marker",icon:"🚩",color:"#ef4444",desc:"Banner/totem/marker on a rise; faction sign, warning, or shrine.",weight:1},
  {id:"movement",label:"Herd/Caravan",icon:"🦌",color:"#22c55e",desc:"Moving mass: herd, caravan, warband; dust plume or trampled swath.",weight:1},
  {id:"earthworks",label:"Earthworks",icon:"⛏️",color:"#a16207",desc:"Fresh earth/clear-cut: dig site, logging, or burrow collapse.",weight:1},
  {id:"fortress",label:"Fortress",icon:"🏰",color:"#64748b",desc:"Fortified stronghold; towers and walls break the horizon.",weight:1},
  {id:"weather",label:"Weather",icon:"⛈️",color:"#38bdf8",desc:"Weather boundary: storm wall, sudden snow line, or miasma bank.",weight:1},
  {id:"geomancy",label:"Stones",icon:"🗿",color:"#0ea5e9",desc:"Standing stones/geometry; ritual circle or survey obelisk.",weight:1},
  {id:"signal",label:"Signal",icon:"📡",color:"#f97316",desc:"Signal: beacon flare, semaphore, mirrored flash, or smoke puffs.",weight:1},
  {id:"sky",label:"Sky sign",icon:"🕊️",color:"#60a5fa",desc:"Sky sign: large bird/wyvern/skyship, cloud vortex, aurora ribbon.",weight:1},
  {id:"sound",label:"Sound",icon:"🔔",color:"#10b981",desc:"Sound carries: drums, chanting, machinery, distant bells.",weight:1},
  {id:"scent",label:"Scent",icon:"〰️",color:"#0ea5a4",desc:"Scent carries: brine, incense, rot, ozone, woodsmoke.",weight:1},
  {id:"route",label:"Route",icon:"🛤️",color:"#a3e635",desc:"Route feature: road cut, switchback, ford, or rope bridge.",weight:1},
  {id:"settlement",label:"Settlement",icon:"🏘️",color:"#f43f5e",desc:"Settlement edge: tilled fields, palisade line, rooftops, or piers.",weight:1},
  {id:"landmark",label:"Landmark",icon:"⛰️",color:"#a78bfa",desc:"Colossal landmark: titan bones, giant tree, or cliff carving.",weight:1},
  {id:"hazard",label:"Hazard",icon:"⚠️",color:"#f59e0b",desc:"Immediate hazard: sinkhole, slide, lava tongue, waking colossus.",weight:1},
];
...
const state = {
  ...
  turn: 0,
  seed: "",
  // starting origin (randomized at boot & via Reset Start)
  startOrigin: {q:0, r:0},
  options: (function(){
    try{
...
function rngTag(tag){ return seededRandom((state.seed||"") + "|" + tag); }
...
const GM_TABLES = {
  ...
  hook: ["Missing courier’s trail","Ransom exchange at dusk","Rumors of a hidden cache","A feud about to explode","A relic wanted by two factions","Strange dreams tied to this place","A map fragment surfaces","A guardian demands tribute","An omen carved into stone","A cursed object seeks a bearer"],
...
/* ---------------- Map & exploration ---------------- */
/* ---- Patrol helpers (new behavior) ---- */
function clearAllPatrols(){
  state.map.forEach((hex)=>{ if(hex && hex.patrol){ hex.patrol.present = false; } });
}
function spawnPatrolOnDiscovery(hex){
  // Only called once, at discovery time of a *new* hex
  if(!hex || !state.patrolsEnabled) return;
  const prng = seededRandom((state.seed||"") + "|patrol|discover|" + hex.q + "," + hex.r);
  const pRoll = Math.floor(prng()*100);
  hex.patrol.present = pRoll < (Number(state.patrolSpawn)||0);
}

function movePatrols(){
  if(!state.patrolsEnabled) return;
  const moves = [];
  state.map.forEach((hex, k)=>{
    if(hex && hex.patrol && hex.patrol.present){
      const rng = seededRandom((state.seed||"") + "|pmove|" + state.turn + "|" + hex.q + "," + hex.r);
      const mRoll = Math.floor(rng()*100);
      if(mRoll < (Number(state.patrolMove)||0)){
        const dirs = neighborAxials(state.orientation);
        const choice = Math.floor(rng()*dirs.length);
        const [dq,dr] = dirs[choice];
        const tq = hex.q + dq, tr = hex.r + dr;
        const tk = key(tq,tr);
        ensureHex(tq,tr);
        moves.push({from:k, to:tk});
      }
    }
  });
  moves.forEach(({from,to})=>{
    const hFrom = state.map.get(from);
    const hTo = state.map.get(to);
    if(hFrom){ hFrom.patrol = {present:false}; state.map.set(from, hFrom); }
    if(hTo){ if(!hTo.patrol) hTo.patrol={present:false}; hTo.patrol.present = true; state.map.set(to, hTo); }
  });
}
...
function ensureHex(q,r){
  ...
  if(!state.map.has(k)){
    ...
    const hex = {q,r, discovered:true, revealed:false, sight, details, hiddenByFog:false, patrol:{present:false}};
    state.map.set(k, hex);
  } else {
    const hex = state.map.get(k);
    if(hex && !('patrol' in hex)) hex.patrol = {present:false};
  }
...
function discoverNeighborsFor(q,r){
  ...
  dirs.forEach(([dq,dr])=>{
    ...
    if(!state.map.has(k)){
      ...
      const hex = {q:nq,r:nr, discovered:true, revealed:false, hiddenByFog, sight, details, patrol:{present:false}};
      // Spawn patrol *only at discovery* if enabled
      spawnPatrolOnDiscovery(hex);
      state.map.set(k, hex);
    }
  });
}
...
function generateNeighbors(){
  ...
  dirs.forEach(([dq,dr],i)=>{
    ...
    results.push({
      ...
      obscured,
      hasPatrol: !!(hex.patrol && hex.patrol.present)
    });
  });
...
function renderLegend(){
  ...
  const psw = el("span",{class:"swatch"}); psw.style.background = "#64748b";
  L.appendChild(el("div",{class:"legend-item"}, psw, `🛡️ Patrol (overlay)`));
}
...
function renderOptions(){
  ...
  arr.forEach((o,idx)=>{
    ...
    const row = el("div",{class:"rowline"},
      ...
      el("input",{type:"number",step:"0.1",min:"0",value:o.weight, oninput:(e)=>{
        o.weight=Number(e.target.value||0); persistConfig();
        const arr = safeOptions();
      ...
      el("button",{class:"btn rm", onclick:()=>{ if(safeOptions().length>1){ state.options.splice(idx,1); persistConfig(); if(state.mode==="weighted"){ generateNeighbors(); } render(); } }}, "🗑️")
    );
...
function renderCards(){
  ...
  six.forEach((r,i)=>{
    ...
    const opt = masked ? {icon:"🌫️", label:"Obscured", desc:"Hidden by fog. Travel to reveal.", color:"#94a3b8"} : r.option;
    const card=el("div",{class:"result", id:"card-"+i, "data-key":key(r.q,r.r), "data-dir": r.direction, style:`border-top:3px solid ${dirColor}`},
      ...
      el("div",{class:"muted",style:"margin:6px 0 6px 0"}, masked
         ...
         : (state.mode==="d20" ? `d20=${r.d20}${r.d20Other?` (${state.visibility==="adv"?"adv":"disadv"} other ${r.d20Other})`:""}`
                               : `weighted pick • ~${chance.toFixed(1)}%`)),
      el("div",{}, opt.desc),
      ...
      (!masked && r.hasPatrol)? el("div",{class:"muted"}, `🛡️ Patrol present`): null
    );
...
function renderGM(){
  ...
  const travelHint = adj ? `<button class="btn primary" id="travelHereBtn">🧭 Travel here</button>` : `<span class="muted">Select an adjacent hex to travel.</span>`;
  if(!hex.revealed && !(hex.q===state.current.q && hex.r===state.current.r)){
    tgt.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:8px">
        <div style="font-size:14px">🌫️ <strong>Obscured by fog</strong> <span class="muted">at</span> (${q},${r})</div>
        <div>${travelHint}</div>
      </div>
    ...
    tgt.innerHTML = `
      ...
      <div class="gm-line"><div class="gm-label">Patrol</div><div class="gm-value">${(hex.patrol && hex.patrol.present) ? "Present" : "None"}</div></div>
    `;
  }
  ...
  if(btn){
    btn.addEventListener("click", ()=>{
      ...
      movePatrols();
      generateNeighbors(); centerViewOn(q,r); render();
      if(state.patrolsEnabled && h.patrol && h.patrol.present){ notify("Patrol encountered here!", "danger"); }
    });
  }
}
...
function renderMap(){
  ...
  state.map.forEach((hex, k)=>{
    ...
    const masked = !!(hex.hiddenByFog && !hex.revealed);
    ...
    iconText.textContent = masked ? "？" : (hex.sight?.option?.icon || "❓");
    g.appendChild(iconText);
    // Patrol overlay (hide if masked)
    if(state.patrolsEnabled && hex.patrol && hex.patrol.present && !masked){
      const patrolIcon = document.createElementNS(svgNS,"text");
      patrolIcon.setAttribute("x",px);
      patrolIcon.setAttribute("y",py-14);
      patrolIcon.setAttribute("text-anchor","middle");
      patrolIcon.setAttribute("font-size","14");
      patrolIcon.setAttribute("fill","#fff");
      patrolIcon.setAttribute("pointer-events","none");
      patrolIcon.textContent = "🛡️";
      g.appendChild(patrolIcon);
    }
  });
...
// --- Patrol controls wiring (changed to reset behavior) ---
(function(){
  const en = document.getElementById("patrolsEnabled");
  const sp = document.getElementById("patrolSpawn");
  const mv = document.getElementById("patrolMove");
  const spPct = document.getElementById("patrolSpawnPct");
  const mvPct = document.getElementById("patrolMovePct");
  function updateDisabled(){
    const on = !!state.patrolsEnabled;
    if(sp) sp.disabled = !on;
    if(mv) mv.disabled = !on;
    if(spPct) spPct.style.opacity = on? "1.0":"0.5";
    if(mvPct) mvPct.style.opacity = on? "1.0":"0.5";
  }
  if(en && sp && mv && spPct && mvPct){
    en.checked = ('patrolsEnabled' in state) ? !!state.patrolsEnabled : true;
    sp.value = Number(state.patrolSpawn)||0; spPct.textContent = (Number(state.patrolSpawn)||0) + "%";
    mv.value = Number(state.patrolMove)||0; mvPct.textContent = (Number(state.patrolMove)||0) + "%";
    updateDisabled();
    en.addEventListener("change", ()=>{
      state.patrolsEnabled = !!en.checked;
      clearAllPatrols(); // Reset or delete all current patrols
      updateDisabled();
      renderMap(); renderCards(); renderLegend();
      notify(state.patrolsEnabled ? "Patrols enabled: will appear only in *newly discovered* spaces." : "Patrols disabled: all existing patrols removed.", state.patrolsEnabled ? "" : "danger");
    });
    sp.addEventListener("input", ()=>{ state.patrolSpawn = Number(sp.value)||0; spPct.textContent = state.patrolSpawn + "%"; renderLegend(); });
    mv.addEventListener("input", ()=>{ state.patrolMove = Number(mv.value)||0; mvPct.textContent = state.patrolMove + "%"; });
  }
})();

// --- Fog controls wiring / Toasts ---
(function(){
...
function randomStartOrigin(){
  // Deterministic given seed; otherwise random
  const rng = seededRandom((state.seed||"") + "|start");
  // pick within radius 4
  const radius = 4;
  // Sample axial coords uniformly-ish within radius
  const candidates = [];
  for(let q=-radius;q<=radius;q++){
    for(let r=max(-radius,-q-radius); r<=min(radius, -q+radius); r++){
      const s = -q - r;
      if(Math.max(Math.abs(q),Math.abs(r),Math.abs(s))<=radius) candidates.push({q,r});
    }
  }
  const idx = Math.floor(rng()*candidates.length);
  return candidates[idx] || {q:0,r:0};
}

// world reset with new random start
function resetWorldWithRandomStart(){
  state.map.clear();
  state.turn = 0;
  state.selectedKey = null;
  state.startOrigin = randomStartOrigin();
  state.current = {q: state.startOrigin.q, r: state.startOrigin.r};
  ensureHex(state.current.q, state.current.r);
  const origin = state.map.get(key(state.current.q, state.current.r)); if(origin){ origin.revealed = true; state.map.set(key(state.current.q,state.current.r), origin); }
  discoverNeighborsFor(state.current.q, state.current.r);
  generateNeighbors();
  centerViewOn(state.current.q, state.current.r);
  render();
  notify(`New start at (${state.current.q},${state.current.r}).`, "");
}

// Seed changes should offer re-randomized start when pressing Reset Start
document.getElementById("resetStart").addEventListener("click", ()=>{
  if(confirm("Randomize starting hex and reset the world? (Patrols follow current settings)")){
    resetWorldWithRandomStart();
  }
});
...
// Migrate persisted 'Patrol' option -> 'Fortress'
(function(){
  let changed=false;
  (state.options||[]).forEach(o=>{
    if(o && (o.id==='patrol' || String(o.label||'').toLowerCase()==='patrol')){
      o.id='fortress'; o.label='Fortress'; o.icon='🏰';
      if(!o.desc || /patrol/i.test(String(o.desc))) o.desc='Fortified stronghold; towers and walls break the horizon.';
      changed=true;
    }
  });
  if(changed){ try{ persistConfig(); }catch(_){} }
})();
...
// Helpers for Math.min/Math.max when using in for-loops
