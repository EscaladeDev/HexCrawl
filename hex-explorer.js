...
/* ---------------- Map & exploration ---------------- */
/* ---- Patrol helpers ---- */
function rngForTag(tag,q,r){
  return seededRandom((state.seed||"") + "|" + String(tag||"") + "|" + q + "," + r + "|turn=" + String(state.turn||0));
}
function movePatrols(){
  ...
  moves.forEach(({from,to})=>{
    ...
    if(hTo){ hTo.patrol = {present:true}; state.map.set(to, hTo); }
  });
}
...
function ensureHex(q,r){
  ...
  if(!state.map.has(k)){
    ...
    // Patrol spawn (stable per hex & seed), only if enabled
    let patrolPresent = false;
    if(state.patrolsEnabled){
      try{
        const prng = seededRandom((state.seed||"") + "|patrol|" + q + "," + r);
        const pRoll = Math.floor(prng()*100);
        patrolPresent = pRoll < (Number(state.patrolSpawn)||0);
      }catch(_){}
    }
    state.map.set(k, {q,r, discovered:true, revealed:false, sight, details, patrol:{present:patrolPresent}});
  } else {
...
function discoverNeighborsFor(q,r){
  ...
  dirs.forEach(([dq,dr])=>{
    ...
    if(!state.map.has(k)){
      ...
      if(state.patrolsEnabled){
        try{
          const prng = seededRandom((state.seed||"") + "|patrol|" + nq + "," + nr);
          const pRoll = Math.floor(prng()*100);
          patrolPresent = pRoll < (Number(state.patrolSpawn)||0);
        }catch(_){}
      }
      state.map.set(k, {q:nq,r:nr, discovered:true, revealed:false, hiddenByFog, sight, details, patrol:{present:patrolPresent}});
    }
  });
}
...
function renderMap(){
  ...
  state.map.forEach((hex, k)=>{
    ...
    if(state.patrolsEnabled && hex.patrol && hex.patrol.present){
      const ringEntry = state._adjMap ? state._adjMap.get(k) : null;
    const masked = !!(hex.hiddenByFog && !hex.revealed);
      if(!masked){
...
// --- Patrol helpers ---
function recalcPatrolPresence(){
  // Recompute patrol presence for all discovered hexes based on current controls
  state.map.forEach((hex, k)=>{
    if(!hex) return;
    if(!hex.patrol) hex.patrol = {present:false};
    if(!state.patrolsEnabled){
      hex.patrol.present = false;
      return;
    }
    try{
      const prng = seededRandom((state.seed||"") + "|patrol|" + hex.q + "," + hex.r);
      const pRoll = Math.floor(prng()*100);
...
(function(){
  ...
  if(en && sp && mv && spPct && mvPct){
    ...
    updateDisabled();
    // listeners
    en.addEventListener("change", ()=>{ state.patrolsEnabled = !!en.checked; updateDisabled(); recalcPatrolPresence(); renderMap(); renderCards(); renderLegend(); });
    sp.addEventListener("input", ()=>{ state.patrolSpawn = Number(sp.value)||0; spPct.textContent = state.patrolSpawn + "%"; recalcPatrolPresence(); renderMap(); renderLegend(); });
    mv.addEventListener("input", ()=>{ state.patrolMove = Number(mv.value)||0; mvPct.textContent = state.patrolMove + "%"; renderMap(); });
  }
})();
;
